...
BUTTON_TO_ACTION = {
    ...
    "BUTTON4":    ["LOW_PUNCH"],
    "BUTTON5":    ["LOW_KICK"],
    # Selon la version du XML, “BUTTON6” peut s’appeler "BLOCK_2" ou "RUN"
    "BUTTON6":    ["BLOCK_2", "RUN"],
}
...
<!-- This file est autogenerated; comments and unknown tags seront supprimés -->
...
"""
    ...
    # Chaque élément de ports est un <port … type="P1_HIGH_PUNCH" mask="16" …/>
    ...
    for player_num in range(1, players_count + 1):
        ...
        if is_mk:
            ...
            #       pour chaque bouton, on reprend tag/mask/defvalue du port d’origine,
            #       on fixe le « type » P{player}_BUTTON{<num direct>} sans tenir compte du remap,
            #       mais le JOYCODE à l’intérieur (newseq.text) utilisera remap_type[phys].
            for button_name, possible_actions in BUTTON_TO_ACTION.items():
                # possible_actions est une liste (ex: ["BLOCK_2", "RUN"] pour BUTTON6, ["HIGH_PUNCH"] pour BUTTON1, etc.)
                btn_elem = None
                for action in possible_actions:
                    # on cherche un <button function="…"> correspondant à l’une des variantes
                    for b in layout.findall('button'):
                        func = b.get('function', '').upper().replace(' ', '_')
                        if func == action:
                            # si color="Black", on skippe
                    ...
                    if btn_elem is not None:
                        break   # on a trouvé un match pour cette BUTTON<n>, on sort de la boucle actions

                if btn_elem is None:
                    # ce BUTTON<n> (et ses variantes) n’existe pas dans le XML → on passe au suivant
                    continue

                phys = int(btn_elem.get('physical'))
                # on récupère le port d’origine, par exemple “P1_HIGH_PUNCH” ou “P1_RUN” ou “P1_BLOCK_2”
                # ATTENTION : si la variante trouvée est "RUN", il faut que dans ports_by_type il y ait
                # un port nommé "P{player}_RUN" ! Sinon, MAME ne trouvera pas le <port> d’origine.
                ...
                # On garde le même “BUTTON<n>” que dans le fichier inputs, c’est-à-dire button_name
                ...
                # SEULS LE JOYCODE change en fonction du remap_type[phys]
        ...
        # (4) Traitement “normaux” (punch-kick, tributton, etc.)
        player_ports = [
            p_elem
            for p_elem in ports
        ...
        for btn in layout.findall('button'):
            ...
            # (4.3) Bouton START (hors MK)
                ...
                if port_key not in ports_by_type:
                    logger.error(f"Missing port pour START_{player_num}")
                    continue
                port = ports_by_type[port_key]
                type_index = phys
                add_button(inp, port, btn, es_map, player_num, type_index, phys)
